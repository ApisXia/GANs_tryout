...
#Define config class
class Config(object):
    def __init__(self, batch_size, latent_size, lr_G, lr_D, sign_real, sign_fake, critic_num, epoch_num, alpha, save_per_epoch):
        ...
        self.lr_G = lr_G
        self.lr_D = lr_D
        self.sign_real = sign_real
        self.sign_fake = sign_fake
    ...
    def outfig(self):
        ...
        print('Sign_real:      %10s' % self.sign_real)
        print('Sign_fake:      %10s' % self.sign_fake)
    ...
    def discriminator(self, inputs, y=None, reuse=False, is_training=False, name='Discriminator'):
        with tf.variable_scope(name) as scope:
            ...
            with tf.name_scope('D_layer4'):
                ...
                d_4 = layer.fully_connected(inputs=d_4, 
                                            num_outputs=1, 
                                            activation_fn=None, 
                                            scope='Linear')
...
#                self.G_loss = tf.reduce_mean(tf.scalar_mul(-1, self.D_fake_logits))
                self.G_loss = tf.reduce_mean(tf.square(tf.subtract(self.D_fake_logits, self.config.sign_real)))
...
#                self.D_loss_fake = tf.reduce_mean(self.D_fake_logits)
                self.D_loss_fake = tf.reduce_mean(tf.square(tf.subtract(self.D_fake_logits, self.config.sign_fake)))
#                self.D_loss_fake = tf.reduce_mean(
#                        tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_fake_logits, 
#                                                                labels=tf.zeros_like(self.D_fake)))
#                self.D_loss_real = tf.reduce_mean(tf.scalar_mul(-1, self.D_real_logits))
                self.D_loss_real = tf.reduce_mean(tf.square(tf.subtract(self.D_real_logits, self.config.sign_real)))
