...
#Define config class
class Config(object):
    def __init__(self, batch_size, latent_size, lr_G, lr_D, beta1, epoch_num, alpha, save_per_epoch):
    ...
    def build(self):
        ...
        with tf.name_scope('Loss'):
            ...
            with tf.name_scope('D_loss'):
                ...
                self.D_loss_sum_real = tf.summary.scalar("D_loss_real", self.D_loss_real)
                self.D_loss_sum_fake = tf.summary.scalar("D_loss_fake", self.D_loss_fake)
...
#        self.D_trainable_vars = [var for var in t_vars if "Discriminator" in var.name]
    ...
    def train(self):
        ...
        with tf.control_dependencies(self.D_update_ops):
            D_opt = tf.train.AdamOptimizer(learning_rate=self.config.lr_D, beta1=self.config.beta1) \
        ...
        with tf.control_dependencies(self.G_update_ops):
            G_opt = tf.train.AdamOptimizer(learning_rate=self.config.lr_G, beta1=self.config.beta1) \
